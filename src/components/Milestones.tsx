        ...
        <AnimatePresence mode="popLayout">
          {sortedMilestones.map((milestone) => (
            <motion.div
              ...
              whileHover={{ y: -2 }}
              className="relative overflow-hidden bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group"
            >
              {/* Background Glow */}
              <div className="absolute -right-4 -top-4 w-24 h-24 bg-indigo-50 dark:bg-indigo-900/20 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity" />
              
              <div className="flex gap-4 relative z-10">
                <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-center border border-gray-100 dark:border-gray-700 group-hover:bg-white dark:group-hover:bg-gray-700 group-hover:border-indigo-100 dark:group-hover:border-indigo-900 transition-colors">
                  {iconMap[milestone.icon] || <Medal className="w-5 h-5 text-gray-400 dark:text-gray-500" />}
                </div>
                ...
                <div className="flex-grow min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {milestone.type === 'streak' && (
                      <span className="flex-shrink-0 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/40 px-2 py-0.5 rounded-full">
              ...
              {milestone.type === 'streak' && (
                <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-50 dark:bg-gray-800">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: '100%' }}
                    transition={{ duration: 1, delay: 0.5 }}
                    className="h-full bg-indigo-500 dark:bg-indigo-400 opacity-20"
                  />
                </div>
              )}
            </motion.div>
